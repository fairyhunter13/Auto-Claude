...
### Epic 9: Comprehensive Testing (P0 Quality)
Establish complete test coverage for all FRs and NFRs through automated testing.
**NFRs covered:** NFR1-NFR27 (all)
**FRs validated:** FR1-FR48 (all traced to tests)
...
Establish comprehensive test coverage for all functional and non-functional requirements, ensuring Auto-BMAD quality through automated backend integration tests, E2E user journey tests, and systematic FR/NFR validation.

**NFRs covered:** NFR1-NFR27 (all performance, usability, reliability, compatibility, security, accessibility)
**FRs validated:** FR1-FR48 (all functional requirements traced to tests)

### Story 9.1: FR/NFR Test Assessment Matrix

As a test architect,
I want a comprehensive test assessment matrix,
So that every requirement has traceable test coverage.

**Acceptance Criteria:**

**Given** the PRD contains 48 FRs and 27 NFRs
**When** I create the test assessment matrix
**Then** every FR has at least one mapped test

**And** every NFR has validation criteria defined

**And** the matrix identifies:
- Test type (unit, integration, E2E)
- Test location (file path)
- Coverage status (covered, partial, missing)
- Priority (P0, P1, P2)

**And** gaps are clearly identified for remediation
...
As a developer,
I want backend integration tests without UI dependency,
So that BMAD core flows can be validated independently.

**Acceptance Criteria:**

**Given** the BMAD main process modules exist
**When** I create backend integration tests
**Then** tests exist for:

**Status Manager Tests:**
- Read bmm-workflow-status.yaml correctly
- Write status with atomic saves
...
**OpenCode Spawn Tests:**
- Spawn OpenCode process correctly
- Pass agent and workflow parameters
- Stream stdout/stderr
- Handle process termination

**And** all tests pass without UI or Electron renderer
...
As a test architect,
I want full workflow lifecycle validation,
So that the complete BMAD methodology flow works correctly.

**Acceptance Criteria:**

**Given** the BMAD workflow system exists
**When** I create lifecycle tests
**Then** tests validate:

**Phase Transition Tests:**
- Phase 1 → 2 transition works
- Phase 2 → 3 requires PRD
- Phase 3 → 4 requires gate check
- Phase reset clears downstream

**Workflow Execution Tests:**
- Workflow starts correctly
- Status updates during execution
- Completion detected accurately
- Artifacts recorded in status

**Human Review Tests:**
- Review decisions saved
- Decision persistence across restart
- Review blocking works correctly
- Override confirmation required

**And** tests are independent of UI rendering

---

### Story 9.4: E2E Test Automation with Video Recording

As a tester,
I want headed E2E tests with video recording,
So that I can visually verify user interactions.

**Acceptance Criteria:**

**Given** Playwright is configured for E2E tests
**When** I configure headed mode
**Then** tests run with visible browser window

**And** video recordings are captured for all tests

**And** trace files are generated for debugging

**And** tests can be run with:
- `npx playwright test --headed` (visible mode)
- `npx playwright test` (CI mode with video)

**And** test reports include:
...
I want comprehensive user journey tests,
So that critical paths always work correctly.

**Acceptance Criteria:**

**Given** the 3 user journeys from PRD exist
**When** I create the regression suite
**Then** Journey 1 (New Project Setup) tests include:
- Create new greenfield project
- Initialize BMAD directories
- Start first workflow (brainstorm)
- View generated artifact

**And** Journey 2 (Architecture Design) tests include:
- Start architecture workflow
- Agent persona activates
- Architecture.md generated
- Gate check runs

**And** Journey 3 (Sprint Execution) tests include:
- Sprint planning workflow
- Story selection from backlog
- Dev story execution
- Code review completion

**And** all journeys have video recordings
...
So that NFR1-NFR5 are verified.

**Acceptance Criteria:**

**Given** performance NFRs are defined
**When** I create performance tests
**Then** tests validate:

**NFR1:** Application launches in < 5 seconds
- Measure cold start time
- Measure warm start time
- Alert if threshold exceeded

**NFR2:** Workflow list loads in < 1 second
- Measure initial load time
- Measure subsequent load time

**NFR3:** Terminal streams with no visible lag
...
**NFR5:** Artifact renders in < 2 seconds
...
### NFR Coverage: 100% (with Epic 9)
All 27 non-functional requirements validated through Epic 9 stories.
...
- Epic 9: Testing - Depends on Epics 1-8 (validation layer)
...
**P0 (Quality):** Epic 9 (parallel with implementation)
