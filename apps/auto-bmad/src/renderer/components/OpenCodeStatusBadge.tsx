...
import type { OpenCodeVersionInfo, OpenCodeInstallationInfo } from "../../shared/types/cli";
...
/**
 * OpenCode CLI status badge for the sidebar.
 * Shows installation status and provides quick access to install/update.
 */
export function OpenCodeStatusBadge({ className }: OpenCodeStatusBadgeProps) {
  ...
  const [installations, setInstallations] = useState<OpenCodeInstallationInfo[]>([]);
  ...
  const fetchVersions = useCallback(async () => {
    if (!window.electronAPI?.getOpenCodeVersions) {
      return;
    }
    ...
    try {
      const result = await window.electronAPI.getOpenCodeVersions();
      if (result.success && result.data) {
  ...
  const fetchInstallations = useCallback(async () => {
    if (!window.electronAPI?.getOpenCodeInstallations) {
      return;
    }
    ...
    try {
      const result = await window.electronAPI.getOpenCodeInstallations();
      if (result.success && result.data) {
  ...
  // Fetch versions when popover opens and OpenCode is installed
  useEffect(() => {
  ...
  const performVersionSwitch = async () => {
    ...
    try {
      if (!window.electronAPI?.installOpenCodeVersion) {
        setInstallError("Version switching not available");
        return;
      }

      const result = await window.electronAPI.installOpenCodeVersion(selectedVersion);
      ...
      console.error("Failed to switch OpenCode version:", err);
  ...
  const performPathSwitch = async () => {
    ...
    try {
      if (!window.electronAPI?.setOpenCodeActivePath) {
        setInstallError("Path switching not available");
        return;
      }

      const result = await window.electronAPI.setOpenCodeActivePath(selectedInstallation);
      ...
      console.error("Failed to switch OpenCode CLI path:", err);
  ...
  const getTooltipText = () => {
    switch (status) {
      case "loading":
        return t("navigation:openCode.checking", "Checking OpenCode...");
      case "installed":
        return t("navigation:openCode.upToDate", "OpenCode is up to date");
      case "outdated":
        return t("navigation:openCode.updateAvailable", "OpenCode update available");
      case "not-found":
        return t("navigation:openCode.notInstalled", "OpenCode not installed");
      case "error":
        return t("navigation:openCode.error", "Error checking OpenCode");
    }
  };
    ...
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent side="right" align="end" className="w-72">
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h4 className="text-sm font-medium">OpenCode CLI</h4>
              <p className="text-xs text-muted-foreground flex items-center gap-1">
                {getStatusIcon()}
                {status === "installed" && t("navigation:openCode.installed", "Installed")}
                {status === "outdated" && t("navigation:openCode.outdated", "Update available")}
                {status === "not-found" && t("navigation:openCode.missing", "Not installed")}
                {status === "loading" && t("navigation:openCode.checking", "Checking...")}
                {status === "error" && t("navigation:openCode.error", "Error")}
              </p>
            </div>
          </div>
          ...
          {versionInfo && status !== "loading" && (
            <div className="text-xs space-y-1 p-2 bg-muted rounded-md">
              {versionInfo.installed && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">
                    {t("navigation:openCode.current", "Current")}:
                  </span>
              ...
              {versionInfo.latest && versionInfo.latest !== "unknown" && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">
                    {t("navigation:openCode.latest", "Latest")}:
                  </span>
              ...
              {versionInfo.path && (
                <div className="flex justify-between items-center gap-2">
                  <span className="text-muted-foreground flex items-center gap-1">
                    ...
                    {t("navigation:openCode.path", "Path")}:
                  </span>
                  <span
              ...
              {lastChecked && (
                <div className="flex justify-between text-muted-foreground">
                  <span>{t("navigation:openCode.lastChecked", "Last checked")}:</span>
          ...
          {/* Version selector - only show when OpenCode is installed */}
          {versionInfo?.installed && (
            <div className="space-y-1.5">
              <label className="text-xs text-muted-foreground">
                {t("navigation:openCode.switchVersion", "Switch Version")}
              </label>
              <Select
              ...
              >
                <SelectTrigger className="h-8 text-xs">
                  <SelectValue
                    placeholder={
                      isLoadingVersions
                        ? t("navigation:openCode.loadingVersions", "Loading versions...")
                        : versionsError
                          ? t("navigation:openCode.failedToLoadVersions", "Failed to load versions")
                          : t("navigation:openCode.selectVersion", "Select version")
                    }
                  />
                </SelectTrigger>
                <SelectContent>
                      ...
                      >
                        ...
                        {isCurrentVersion && (
                          <span className="ml-2 text-muted-foreground">
                            ({t("navigation:openCode.currentVersion", "Current")})
                          </span>
                        )}
                      </SelectItem>
                    );
          ...
          {installations.length > 1 && (
            <div className="space-y-1.5">
              <label className="text-xs text-muted-foreground">
                {t("navigation:openCode.switchInstallation", "Switch Installation")}
              </label>
              <Select
                  ...
                  <SelectValue
                    placeholder={
                      isLoadingInstallations
                        ? t("navigation:openCode.loadingInstallations", "Loading installations...")
                        : installationsError
                          ? t("navigation:openCode.failedToLoadInstallations", "Failed to load installations")
                          : t("navigation:openCode.selectInstallation", "Select installation")
                    }
                  />
                </SelectTrigger>
                <SelectContent>
                    ...
                    >
                      <div className="flex flex-col">
                        ...
                        <span className="text-muted-foreground text-[9px]">
                          {installation.version ? `v${installation.version}` : t("navigation:openCode.versionUnknown", "version unknown")} ({installation.source})
                          {installation.isActive && ` - ${t("navigation:openCode.activeInstallation", "Active")}`}
                        </span>
                      </div>
                    </SelectItem>
                  ))}
          ...
          <Button
            ...
            aria-label={t(
              "navigation:openCode.learnMoreAriaLabel",
              "Learn more about OpenCode (opens in new window)"
            )}
          >
            {t("navigation:openCode.learnMore", "Learn more about OpenCode")}
            <ExternalLink className="h-3 w-3" aria-hidden="true" />
          </Button>

          {/* Installation instructions */}
          <div className="text-xs text-muted-foreground p-2 bg-muted/50 rounded-md">
            <p className="font-medium mb-1">Install via Go:</p>
      ...
      <AlertDialog open={showUpdateWarning} onOpenChange={setShowUpdateWarning}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("navigation:openCode.updateWarningTitle", "Update OpenCode?")}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t(
                "navigation:openCode.updateWarningDescription",
                "Updating will install the latest version of OpenCode. Make sure to save your work before proceeding."
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={performInstall}>
              {t("navigation:openCode.updateAnyway", "Update Anyway")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={showRollbackWarning} onOpenChange={setShowRollbackWarning}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("navigation:openCode.rollbackWarningTitle", "Switch to version {{version}}?", {
                version: selectedVersion,
              })}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t(
                "navigation:openCode.rollbackWarningDescription",
                "Switching versions will install the selected version of OpenCode. Make sure to save your work before proceeding."
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={performVersionSwitch}>
              {t("navigation:openCode.switchAnyway", "Switch Anyway")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AlertDialog open={showPathChangeWarning} onOpenChange={setShowPathChangeWarning}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {t("navigation:openCode.pathChangeWarningTitle", "Switch CLI installation?")}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {t(
                "navigation:openCode.pathChangeWarningDescription",
                "Switching CLI installations will use a different OpenCode binary."
              )}
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={performPathSwitch}>
              {t("navigation:openCode.switchInstallationConfirm", "Switch")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
